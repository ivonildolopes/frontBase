...
export class PdfComponent implements OnInit {
  ...
  teste() {
    this.pdfService.geraPdf().subscribe(res => {
      ...
      //window.open(res.data, '', 'width=800,height=800');
      ...
      //this.openFileForPrint(res.data);
  ...
  exibirRelatorio(res) {
   /* const linkSource =  new Blob(res.data);
        const downloadLink = document.createElement('a');
        const fileURL = URL.createObjectURL(linkSource);
        window.open(fileURL);
        */

       const file = new Blob([res.data.blob], { type: 'application/pdf' });
       const fileURL = URL.createObjectURL(file);
       window.open(fileURL);
